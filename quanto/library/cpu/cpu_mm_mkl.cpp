...
torch::Tensor& _int_mm_out_cpu(const torch::Tensor& self, const torch::Tensor& mat2, torch::Tensor& result) {
    ...
    MKL_INT         m=self.size(0), n=mat2.size(1), k=self.size(1);
    ...
    cblas_gemm_s8u8s32(CblasRowMajor, CblasNoTrans, CblasTrans, offsetc,
                      m, n, k, alpha, (self + 128).data_ptr<int8_t>(), lda, ao, mat2.data_ptr<int8_t>(), ldb, bo, beta, result.data_ptr<int32_t>(), ldc, &co);

    result -= 128 * torch::sum(mat2, 0);
